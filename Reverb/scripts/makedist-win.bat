...
if %1 == 1 (echo Making Reverb Windows DEMO VERSION distribution ...) else (echo Making Reverb Windows FULL VERSION distribution ...)
...
msbuild Reverb.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild Reverb.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Reverb.aaxplugin\Contents\Win32\Reverb.aaxplugin --out .\build-win\aax\bin\Reverb.aaxplugin\Contents\Win32\Reverb.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Reverb.aaxplugin\Contents\x64\Reverb.aaxplugin --out .\build-win\aax\bin\Reverb.aaxplugin\Contents\x64\Reverb.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\Reverb.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\Reverb.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "Reverb Installer" ".\installer\Reverb Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\Reverb Installer.exe" ".\installer\Reverb Demo Installer.exe"
REM -del ".\installer\Reverb Installer.exe"
REM -)
