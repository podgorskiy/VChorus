...
def  main():
  ...
  installer = "\installer\Reverb Installer.exe"
   
  if demo:
    installer = "\installer\Reverb Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Reverb manual.pdf" 
  ]
  ...
  ZIPNAME = "Reverb-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Reverb-v" + FULLVERSIONSTR + "-win-demo.zip"
