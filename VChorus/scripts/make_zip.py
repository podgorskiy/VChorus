...
def  main():
  ...
  installer = "\installer\VChorus Installer.exe"
   
  if demo:
    installer = "\installer\VChorus Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\VChorus manual.pdf" 
  ]
  ...
  ZIPNAME = "VChorus-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "VChorus-v" + FULLVERSIONSTR + "-win-demo.zip"
