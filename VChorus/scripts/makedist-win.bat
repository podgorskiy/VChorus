...
if %1 == 1 (echo Making VChorus Windows DEMO VERSION distribution ...) else (echo Making VChorus Windows FULL VERSION distribution ...)
...
REM - msbuild VChorus-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild VChorus.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild VChorus.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\VChorus.aaxplugin\Contents\Win32\VChorus.aaxplugin --out .\build-win\aax\bin\VChorus.aaxplugin\Contents\Win32\VChorus.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\VChorus.aaxplugin\Contents\x64\VChorus.aaxplugin --out .\build-win\aax\bin\VChorus.aaxplugin\Contents\x64\VChorus.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\VChorus.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\VChorus.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "VChorus Installer" ".\installer\VChorus Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\VChorus Installer.exe" ".\installer\VChorus Demo Installer.exe"
REM -del ".\installer\VChorus Installer.exe"
REM -)
